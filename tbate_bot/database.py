...
def setup_database():
    ...
    if cursor.execute("SELECT COUNT(*) FROM items").fetchone()[0] == 0:
        items_to_insert = [(i['name'], i['description'], i['price'], i['tradeable'], i['item_type'], i.get('emoji')) for i in items.ALL_ITEMS]
        cursor.executemany("INSERT INTO items (name, description, price, tradeable, item_type, emoji) VALUES (?, ?, ?, ?, ?, ?)", items_to_insert)

    if cursor.execute("SELECT COUNT(*) FROM tasks").fetchone()[0] == 0:
        default_tasks = [
            ("Go on 5 expeditions", "expedition", 5),
            ("Sell 100 beast cores", "sell_cores", 100),
            ("Win 2 minigames", "win_minigame", 2),
            ("Earn 1000 gold from selling", "earn_gold", 1000),
            ("Gain 500 XP", "gain_xp", 500),
            ("Craft an item", "craft_item", 1)
        ]
    ...
    if cursor.execute("SELECT COUNT(*) FROM recipes").fetchone()[0] == 0:
        recipes_to_add = {
            "Greater Stamina Potion": [("Stamina Potion", 2), ("Glow Moss", 5)],
            "Superior Stamina Potion": [("Greater Stamina Potion", 2), ("Moonshade Petal", 3)],
            "Beast Lure": [("Mana Infused Hide", 3), ("Broken Beast Horn", 10)],
            "Small XP Scroll": [("Pure Mana Crystal", 1), ("Glow Moss", 10)],
            "Medium XP Scroll": [("Small XP Scroll", 2), ("Griffin Feather", 5)],
        }
    ...
    if cursor.execute("SELECT COUNT(*) FROM bonds").fetchone()[0] == 0:
        default_bonds = [
            ("Sylvie", "Epic", "🐲", "xp_boost", 0.05, "A loyal dragon who boosts XP gain by 5%."),
            ("Regis", "Epic", "🐺", "gold_boost", 0.10, "A snarky weapon who boosts Gold gain by 10%."),
            ("Caera's Companion", "Rare", "🦅", "energy_regen_boost", 0.10, "A steadfast hawk who boosts energy regen rate by 10%."),
            ("Gideon", "Uncommon", "🔬", "crafting_cost_reduction", 0.05, "An eccentric inventor who reduces material costs for crafting by 5% (not yet implemented)."),
        ]
...
def get_player(user_id):
    ...
    if energy_to_add > 0:
        current_energy = player['energy']; max_energy = calculate_max_energy(player['core_level'])
        new_energy = min(max_energy, current_energy + energy_to_add)
        if new_energy > current_energy:
            time_used = (new_energy - current_energy) * current_regen_rate
            new_timestamp = last_update + time_used
...
def get_inventory(user_id):
    conn = sqlite3.connect('players.db'); conn.row_factory = sqlite3.Row; cursor = conn.cursor()
    cursor.execute("SELECT i.item_id, i.name, i.description, inv.quantity, i.emoji FROM inventory inv JOIN items i ON inv.item_id = i.item_id WHERE inv.user_id = ? AND inv.quantity > 0 ORDER BY i.name", (user_id,))
